...
{
	class AccentBoxTagger : ITagger<AccentBoxOccurrences>
	{
		...
		public AccentBoxTagger(ITextView view, ITextBuffer sourceBuffer, ITextSearchService textSearchService, IClassificationTypeRegistryService typeService)
		{
			this.View = view;
		...
		public IEnumerable<ITagSpan<AccentBoxOccurrences>> GetTags(NormalizedSnapshotSpanCollection spans)
		{
			...
			{
				yield return new TagSpan<AccentBoxOccurrences>(currentWord, new AccentBoxOccurrences());
			}

			// Second, yield all the other words in the file 
			foreach (SnapshotSpan span in NormalizedSnapshotSpanCollection.Overlap(spans, wordSpans))
			{
				yield return new TagSpan<AccentBoxOccurrences>(span, new AccentBoxOccurrences());
			}
		}
	}
}